const PrivacyPolicy = () => {
  return (
    <section className="flex flex-col items-center justify-center font-inter">
      <section className="py-24 flex flex-col w-full max-w-[720px] gap-6">
        <div className="w-full h-fit flex flex-col items-center justify-center gap-3">
          <p className="text-primary text-base font-semibold">
            Current as of 20 Jan 2024
          </p>
          <h2 className="text-5xl font-semibold">Privacy Policy</h2>
        </div>
        <p className="text-xl text-custom-gray-600 text-center">
          Your privacy is important to us at shop4them. We respect your privacy
          regarding any information we may collect from you across our website.
        </p>
      </section>
      <section className="w-full h-fit max-w-[720px] flex flex-col items-center justify-center gap-12">
        <p className="text-lg text-custom-gray-600">
          Mi tincidunt elit, id quisque ligula ac diam, amet. Vel etiam
          suspendisse morbi eleifend faucibus eget vestibulum felis. Dictum quis
          montes, sit sit. Tellus aliquam enim urna, etiam. Mauris posuere
          vulputate arcu amet, vitae nisi, tellus tincidunt. At feugiat sapien
          varius id. Eget quis mi enim, leo lacinia pharetra, semper. Eget in
          volutpat mollis at volutpat lectus velit, sed auctor. Porttitor fames
          arcu quis fusce augue enim. Quis at habitant diam at. Suscipit
          tristique risus, at donec. In turpis vel et quam imperdiet. Ipsum
          molestie aliquet sodales id est ac volutpat.{" "}
        </p>
        <div className="w-full h-fit flex flex-col items-start justify-center gap-6">
          <h3 className="text-3xl text-custom-gray-900 font-semibold">
            What information do we collect?
          </h3>
          <p className="text-lg text-custom-gray-600">
            Dolor enim eu tortor urna sed duis nulla. Aliquam vestibulum, nulla
            odio nisl vitae. In aliquet pellentesque aenean hac vestibulum
            turpis mi bibendum diam. Tempor integer aliquam in vitae malesuada
            fringilla.
          </p>
          <p className="text-lg text-custom-gray-600">
            Elit nisi in eleifend sed nisi. Pulvinar at orci, proin imperdiet
            commodo consectetur convallis risus. Sed condimentum enim dignissim
            adipiscing faucibus consequat, urna. Viverra purus et erat auctor
            aliquam. Risus, volutpat vulputate posuere purus sit congue
            convallis aliquet. Arcu id augue ut feugiat donec porttitor neque.
            Mauris, neque ultricies eu vestibulum, bibendum quam lorem id. Dolor
            lacus, eget nunc lectus in tellus, pharetra, porttitor.
          </p>
          <p className="text-lg text-custom-gray-600">
            Ipsum sit mattis nulla quam nulla. Gravida id gravida ac enim mauris
            id. Non pellentesque congue eget consectetur turpis. Sapien, dictum
            molestie sem tempor. Diam elit, orci, tincidunt aenean tempus. Quis
            velit eget ut tortor tellus. Sed vel, congue felis elit erat nam
            nibh orci.
          </p>
        </div>
        <div className="w-full h-fit flex flex-col items-start justify-center gap-6">
          <h3 className="text-3xl text-custom-gray-900 font-semibold">
            How do we use your information?
          </h3>
          <p className="text-lg text-custom-gray-600">
            Dolor enim eu tortor urna sed duis nulla. Aliquam vestibulum, nulla
            odio nisl vitae. In aliquet pellentesque aenean hac vestibulum
            turpis mi bibendum diam. Tempor integer aliquam in vitae malesuada
            fringilla.
          </p>
          <p className="text-lg text-custom-gray-600">
            Elit nisi in eleifend sed nisi. Pulvinar at orci, proin imperdiet
            commodo consectetur convallis risus. Sed condimentum enim dignissim
            adipiscing faucibus consequat, urna. Viverra purus et erat auctor
            aliquam. Risus, volutpat vulputate posuere purus sit congue
            convallis aliquet. Arcu id augue ut feugiat donec porttitor neque.
            Mauris, neque ultricies eu vestibulum, bibendum quam lorem id. Dolor
            lacus, eget nunc lectus in tellus, pharetra, porttitor.
          </p>
          <p className="text-lg text-custom-gray-600">
            Ipsum sit mattis nulla quam nulla. Gravida id gravida ac enim mauris
            id. Non pellentesque congue eget consectetur turpis. Sapien, dictum
            molestie sem tempor. Diam elit, orci, tincidunt aenean tempus. Quis
            velit eget ut tortor tellus. Sed vel, congue felis elit erat nam
            nibh orci.
          </p>
        </div>
        <div className="w-full h-fit flex flex-col items-start justify-center gap-6">
          <h3 className="text-2xl text-custom-gray-900 font-semibold">
            Do we use cookies and other Letter spacing technologies?
          </h3>
          <p className="text-lg text-custom-gray-600">
            Ipsum sit mattis nulla quam nulla. Gravida id gravida ac enim mauris
            id. Non pellentesque congue eget consectetur turpis. Sapien, dictum
            molestie sem tempor. Diam elit, orci, tincidunt aenean tempus. Quis
            velit eget ut tortor tellus. Sed vel, congue felis elit erat nam
            nibh orci.
          </p>
        </div>
        <div className="w-full h-fit flex flex-col items-start justify-center gap-6">
          <h3 className="text-2xl text-custom-gray-900 font-semibold">
            How long do we keep your information?
          </h3>
          <p className="text-lg text-custom-gray-600">
            Ipsum sit mattis nulla quam nulla. Gravida id gravida ac enim mauris
            id. Non pellentesque congue eget consectetur turpis. Sapien, dictum
            molestie sem tempor. Diam elit, orci, tincidunt aenean tempus. Quis
            velit eget ut tortor tellus. Sed vel, congue felis elit erat nam
            nibh orci.
          </p>
        </div>
        <div className="w-full h-fit flex flex-col items-start justify-center gap-6">
          <h3 className="text-2xl text-custom-gray-900 font-semibold">
            How do we keep your information safe?
          </h3>
          <p className="text-lg text-custom-gray-600">
            Ipsum sit mattis nulla quam nulla. Gravida id gravida ac enim mauris
            id. Non pellentesque congue eget consectetur turpis. Sapien, dictum
            molestie sem tempor. Diam elit, orci, tincidunt aenean tempus. Quis
            velit eget ut tortor tellus. Sed vel, congue felis elit erat nam
            nibh orci.
          </p>
        </div>
        <div className="w-full h-fit flex flex-col items-start justify-center gap-6">
          <h3 className="text-2xl text-custom-gray-900 font-semibold">
            What are your privacy rights?
          </h3>
          <p className="text-lg text-custom-gray-600">
            Ipsum sit mattis nulla quam nulla. Gravida id gravida ac enim mauris
            id. Non pellentesque congue eget consectetur turpis. Sapien, dictum
            molestie sem tempor. Diam elit, orci, tincidunt aenean tempus. Quis
            velit eget ut tortor tellus. Sed vel, congue felis elit erat nam
            nibh orci.
          </p>
        </div>
        <div className="w-full h-fit flex flex-col items-start justify-center gap-6">
          <h3 className="text-2xl text-custom-gray-900 font-semibold">
            How can you contact us about this policy?
          </h3>
          <p className="text-lg text-custom-gray-600">
            Ipsum sit mattis nulla quam nulla. Gravida id gravida ac enim mauris
            id. Non pellentesque congue eget consectetur turpis. Sapien, dictum
            molestie sem tempor. Diam elit, orci, tincidunt aenean tempus. Quis
            velit eget ut tortor tellus. Sed vel, congue felis elit erat nam
            nibh orci.
          </p>
          <ol
            className="text-lg text-custom-gray-600 font-inter list-decimal list-inside"
            type="I"
          >
            <li>Lectus id duis vitae porttitor enim gravida morbi.</li>
            <li>
              Eu turpis posuere semper feugiat volutpat elit, ultrices
              suspendisse. Auctor vel in vitae placerat.
            </li>
            <li>
              Suspendisse maecenas ac donec scelerisque diam sed est duis purus.
            </li>
          </ol>
        </div>
      </section>
    </section>
  );
};

export default PrivacyPolicy;
